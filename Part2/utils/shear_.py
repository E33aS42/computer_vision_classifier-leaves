...
def shear_(path):
	...
	sheared_image = np.zeros((image.shape[0], new_width, image.shape[2]), dtype=np.uint8)
	...
	# Define the shear transformation matrix
	shear_matrix = np.array([[1, shear_factor_x, translation_x], [0, 1, 0]], dtype=np.float32)

	# Apply the shear transformation
	# sheared_image = cv2.warpAffine(image, shear_matrix, (new_width, image.shape[0]), borderValue=(255, 255, 255))
	sheared_image = cv2.warpAffine(image, shear_matrix, (new_width, image.shape[0]), borderValue=(0, 0, 0))
	sheared_image = cv2.resize(sheared_image, (image.shape[0], image.shape[1]))
	# sheared_image = rembg_(sheared_image)
